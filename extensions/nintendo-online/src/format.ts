...
export function htmlToMarkdown(html: string): string {
  ...
  return (
    html
      ...
      .replace(
        /<aside[^>]*class="[^"]*quote[^"]*"[^>]*>([\s\S]*?)<\/aside>/gi,
        (_match: string, content: string) => {
          const titleMatch = content.match(
            /<div[^>]*class="[^"]*title[^"]*"[^>]*>([\s\S]*?)<\/div>(?=\s*<blockquote)/i,
          );
          ...
          if (titleMatch) {
            ...
            const avatarMatch = titleHtml.match(
              /<img[^>]*src\s*=\s*(["'])(.*?)\1/i,
            );
            if (avatarMatch) {
          ...
          if (!avatarUrl) {
            const fallbackAvatarMatch = content.match(
              /<img[^>]*class="[^"]*avatar[^"]*"[^>]*src\s*=\s*(["'])(.*?)\1/i,
            );
      ...
      .replace(/<br\s*\/?>/gi, "\n")
      .replace(
        /<(?:p|div|section|article|header|footer|figure)[^>]*>/gi,
        "\n\n",
      )
      ...
      .replace(
        ...
        (_match: string, content: string) => wrapInline(content, "**"),
      )
      .replace(/<b[^>]*>([\s\S]*?)<\/b>/gi, (_match: string, content: string) =>
        wrapInline(content, "**"),
      )
      .replace(
        /<em[^>]*>([\s\S]*?)<\/em>/gi,
      ...
      .replace(/<i[^>]*>([\s\S]*?)<\/i>/gi, (_match: string, content: string) =>
        wrapInline(content, "*"),
      )
      ...
      .replace(/<img\b[^>]*>/gi, (match: string) => {
        ...
        const dataNameMatch = match.match(
          /\bdata-(?:emoji-name|emoji-shortname)\s*=\s*(["'])(.*?)\1/i,
        );

        const altText = altMatch ? altMatch[2].trim() : "";
        const fallbackText = titleMatch
          ? titleMatch[2].trim()
          : dataNameMatch
            ? dataNameMatch[2].trim()
            : "";
      ...
      .replace(
        ...
        (_match: string, content: string) => {
          const items = content.replace(
            /<li[^>]*>([\s\S]*?)<\/li>/gi,
            (_liMatch: string, li: string) => `- ${li.trim()}\n`,
          );
          return `\n\n${items.trimEnd()}\n\n`;
        },
      )
      .replace(
        ...
        (_match: string, content: string) => {
          ...
          const items = content.replace(
            /<li[^>]*>([\s\S]*?)<\/li>/gi,
            (_liMatch: string, li: string) => `${counter++}. ${li.trim()}\n`,
          );
          return `\n\n${items.trimEnd()}\n\n`;
        },
      )
